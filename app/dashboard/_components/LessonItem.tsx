  ...
  return (
    <Link
      ...
      className={buttonVariants({
        variant: "outline",
        className: cn(
          "w-full p-2 h-auto justify-start transition-all",
          completed &&
            "bg-green-200 border-green-300 hover:bg-green-300 text-green-800 hover:text-green-800 dark:bg-green dark:text-green-400",
          isActive &&
            !completed &&
            "bg-primary/10 dark:bg-primary/20 border-primary/50 dark:border-primary/50 hover:bg-primary/20 dark:hover:bg-primary/30 text-primary"
        ),
      })}
    >
      <div className="flex items-center gap-2 min-w-0">
        <div className="shrink-0">
          {completed ? (
            ...
            >
              <PlayIcon className={cn("size-2 fill-current")} />
            </div>
          )}
        </div>
